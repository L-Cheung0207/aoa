...
describe("ipc schemas", () => {
  ...
  it("rejects empty insert text", () => {
    expect(() => parseInsertTextInput({ text: "   " })).toThrow("Insert text is required");
  });
  ...
  it("rejects a non-string replacement selection guard", () => {
    expect(() =>
      parseReplaceSelectedTextInput({ text: "new text", expectedSelectedText: 42 })
  ...
  it("accepts direct recording mode", () => {
    expect(parseStartRecordingInput({ mode: "direct" })).toEqual({ mode: "direct" });
  });

  it("accepts processSelection with selectedText", () => {
    expect(
      parseStartRecordingInput({ mode: "processSelection", selectedText: "hello" })
